...
---@param line string
---@return docgen.renderer.states_
---@return string?
local interpret_state = function(line)
  ...
  elseif line == "</pre>" or line == "```" then -- FIX: repeating ``` here won't work
...
---@class docgen.renderer.Text
---@field state docgen.renderer.states
---@field paragraphs string[][]
---@field itemizes string[][]
---@field enumerates string[][]
...
function Text:new(input)
  ...
  for _, line in vim.iter(input):flatten() do
    if not obj:is_empty() or line ~= "" then obj:handle_line(line) end
  end
...
function Text:handle_line(line)
  ...
  if new_state == self.state then
    ...
    self:dispatch_new_state(line, new_state)
...
---@param line string
---@param state docgen.renderer.states
function Text:dispatch_new_state(line, state)
  if state == states.PARAGRAPH then
    self:add_to_paragraph(line)
  elseif state == states.NEWLINE then
    self:start_new_paragraph()
  elseif state == states.ITEMIZE then
    self:add_to_itemize(line)
  elseif state == states.ENUMERATE then
    self:add_to_enumerate(line)
  end
end

local rstrip = function(str)
  return str:gsub("%s*$", "")
end
...
function Text:start_new_paragraph(line)
  if #self.paragraphs > 0 then
    self.paragraphs[#self.paragraphs] = rstrip(self.paragraphs[#self.paragraphs])
  end
  table.insert(self.paragraphs, "")
  table.insert(self.order, states.PARAGRAPH)
  if line then self:add_to_paragraph(line) end
end
...
function Text:start_new_itemize(line)
  ...
  if line then self:add_to_itemize(line) end
end
...
function Text:start_new_enumerate(line)
  ...
  if line then self:add_to_enumerate(line) end
end
...
M.render = function(input, prefix, width)
  ...
  local handle = function(string, additional_prefix)
    ...
    if type(string) ~= "table" then string = { string } end
  ...
  for typ, paragraph in text:iter() do
    ...
    elseif typ == states.ITEMIZE then
      for _, str in ipairs(paragraph) do
        local str_start, _ = (function()
          if type(str) == "table" then return str[1]:find("-") end
          return str:find("-")
        end)()
    ...
    elseif typ == states.ENUMERATE then
      ...
      for _, str in ipairs(paragraph) do
        local indent_level, newc = (function()
          if type(str) == "table" then return str[1]:find("[0-9.]+%. ") end
          return str:find("[0-9.]+%. ")
        end)()
        ...
        if not newc then
          error(
            "Invalid enumerate. Enumerates have to end with `dot`. Example: `1. Item` or `1.1. Item`"
          )
        end
        if not maxc[indent_level] then
          maxc[indent_level] = newc
        else
          if newc > maxc[indent_level] then maxc[indent_level] = newc end
        end
      end
    ...
    elseif typ == states.IGNORE then
      for _, str in ipairs(paragraph) do
        local construct = rstrip(prefix .. str)
        table.insert(output, construct)
      end
      if paragraph[1] and vim.trim(paragraph[1]):match("^>") then table.insert(output, "<") end
    else
      text:error()
    end
  end
...
--         if #output == 0 then inner_width = inner_width - #prefix end
